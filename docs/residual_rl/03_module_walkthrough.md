...
**Why a new file at all.** Residual RL needs three things the legacy buffer cannot provide cleanly:

1. **Different horizons for action and reward.** The actor consumes a 4-step action chunk; the critic uses 3-step n-step TD targets. The legacy buffer wired both to a single `action_horizon`, so the only way to get them apart was to oversize one and waste compute.
...
#### `main` `replay_buffer.py` vs `features/residual_rl` `resfit_replay_buffer.py`

Side-by-side breakdown of every knob that differs, with the rationale for the change. Use this table to argue down "should we revert the residual buffer to look more like the old one?" questions in code review.

| Knob | `main` value | `features/residual_rl` value | Why this changed |
|---|---|---|---|
| **Default `capacity`** | `10_000_000` ([`replay_buffer.py:11` on `main`](https://github.com/KyunHwan/online_rl/blob/85fb2e9b44ac08f039f10ca7586e5fd36b36f105/data_bridge/replay_buffer.py#L11)) | `100_000` ([`resfit_replay_buffer.py:14`](../../data_bridge/resfit_replay_buffer.py#L14)) | `LazyMemmapStorage` pre-allocates the *entire* memmap file the first time it sees a row, sized to `capacity` per field. With 10 M rows times three 240×320×3 uint8 images + 24-D proprio + 24-D action ≈ **2.2 GB per camera**, the storage allocates **~7 GB of memmap on first `add()`**. That allocation blocks the labeler for tens of seconds on a fast SSD, longer on the training PC's spinning scratch disk, and frequently filled the `/tmp` partition. At the residual-RL rollout cadence (~1 000 rows/episode), `100_000` rows is **~100 episodes** of buffer — far more than the off-policy critic needs, since old data goes stale as the residual policy diverges from older versions. `100_000` allocates in well under a second and the storage stays comfortably inside `/tmp`. |
| **Default `compile`** | `True` ([`replay_buffer.py:30` on `main`](https://github.com/KyunHwan/online_rl/blob/85fb2e9b44ac08f039f10ca7586e5fd36b36f105/data_bridge/replay_buffer.py#L30)) | `False` ([`resfit_replay_buffer.py:29`](../../data_bridge/resfit_replay_buffer.py#L29)) | `SliceSampler(compile=True)` torch-compiles the sampling kernel. The compile is **deferred until the first `sample()` after the first `add()`**, because the kernel needs the concrete tensor dtypes and shapes that `LazyMemmapStorage` only learns when the first row arrives. In practice this means: as soon as the first sub-episode lands in the buffer, the trainer's `replay_buffer.sample.remote(...)` call hangs for **tens of seconds** while torch traces and compiles the sampler against the freshly-materialized memmap shapes. That stall propagates into the trainer's fill-wait loop, the labeler queue backs up, and the control loop starts dropping sub-episodes. With `compile=False` the sampler runs in eager mode — slightly slower *per* sample, but with **no first-call lag**, so startup is predictable and the labeler → buffer → trainer pipeline reaches steady-state immediately. The throughput cost is negligible compared to the trainer's per-step ResNet34 + Q-ensemble forward passes. Same rationale applies to the legacy buffer (commit `fc177a7`). |
| **Scratch directory** | Relative `tmp/online_rl_auto_data` ([`replay_buffer.py:62` on `main`](https://github.com/KyunHwan/online_rl/blob/85fb2e9b44ac08f039f10ca7586e5fd36b36f105/data_bridge/replay_buffer.py#L62)) | Absolute `/tmp/online_rl_auto_data` ([`resfit_replay_buffer.py:59`](../../data_bridge/resfit_replay_buffer.py#L59)) | Ray actors do not guarantee a fixed working directory — placement decisions and the actor's `runtime_env.working_dir` can land the actor in different cwds across restarts. Relative paths created memmap files next to whichever directory the actor happened to start in, which (a) leaked memmaps into the source tree, (b) caused stale-file conflicts when two actors raced on overlapping cwds, (c) failed silently if the cwd was read-only. Absolute `/tmp/` makes the location unambiguous. |
| **Stale-storage wipe at construction** | None | `shutil.rmtree("/tmp/online_rl_auto_data")` if it exists ([`resfit_replay_buffer.py:53-55`](../../data_bridge/resfit_replay_buffer.py#L53-L55)) + `existsok=True` on the `LazyMemmapStorage` ([`resfit_replay_buffer.py:60`](../../data_bridge/resfit_replay_buffer.py#L60)) | Without the wipe, a prior run's memmap files persist on disk with whatever shape they had then. If you bump `capacity` or change `action_horizon` between runs, the second run's `LazyMemmapStorage` errors with a shape-mismatch on the first `add()`. The branch chose "always start fresh" over "reuse old data": replay-buffer state is not durable across runs anyway, and the wipe makes the failure mode disappear. `existsok=True` exists so that `LazyMemmapStorage` does not refuse to (re-)create the directory after the wipe — without it, the constructor raises if the directory was wiped by the same process moments earlier and not yet GC'd by the OS. |
| **Chunking model** | `action_horizon` + `obs_proprio_history` + `obs_images_history` + `chunking_mode` (`'classic'` or `'lerobot_qchunk'`) ([`replay_buffer.py:23-26`](../../data_bridge/replay_buffer.py#L23-L26)) | `action_horizon` + `reward_horizon` + `obs_subsample_step` ([`resfit_replay_buffer.py:23-25`](../../data_bridge/resfit_replay_buffer.py#L23-L25)) | The imitation-learning buffer needed an action chunk paired with an obs *history*; rewards were not used, so there was no reward horizon. Residual RL flips the model: the actor needs an action *horizon*, the critic needs a *separate* reward horizon for n-step TD, and the obs side only needs a stride to subsample (every 3rd timestep is enough — the residual MLP does not benefit from a denser history). `obs_subsample_step=3` with `action_horizon=4` yields obs offsets `[3, 0]`; the critic uses index `1` as "now" and index `0` as "future." See [05 § replay-buffer args](./05_config_and_hyperparameters.md#replay-buffer-construction-args). |
| **Stored fields** | `action`, `proprio`, `head/left/right`, `reward`, `episode`, `control_mode`, `task_index`, `("next", "done")` | All of the above **plus** `base_policy_action` and `task` (string, `NonTensorData`) | `base_policy_action` is required so the critic and actor agree on what the base policy did at training time (see "Why a new file at all" above). `task` matches the offline LeRobot dataset's `task` key so online + offline batches concatenate without renames. |
| **Sample output keys** | `action`, `labels.reward`, `observation.state`, `observation.images.cam_*` ([`replay_buffer.py:179-202` on `main`](https://github.com/KyunHwan/online_rl/blob/85fb2e9b44ac08f039f10ca7586e5fd36b36f105/data_bridge/replay_buffer.py#L179-L202)) | Same, **plus** `base_policy_action` and `observation.current` (alias of `observation.state`) ([`resfit_replay_buffer.py:154-180`](../../data_bridge/resfit_replay_buffer.py#L154-L180)) | The trainer expects `observation.current` as a separate key (legacy from the imitation-learning recipe that distinguished "current state" from "state history"). The residual recipe does not need the distinction, so the new buffer emits the same tensor under both names rather than forcing a trainer-side rename. |
| **HIL split routing** | `use_hil_buffer` exists in both — same code path, separates `control_mode==1` rows into a second storage. | Same behavior. | Unchanged from the legacy buffer's `use_hil_buffer` flag, just preserved verbatim. |

In one sentence: the new buffer was sized down so the memmap allocates in **<1 s** instead of stalling on first `add()`, compiled-out so the sampler does not hang for tens of seconds on first `sample()`, and re-keyed so the trainer can mix online and offline batches without rename glue.
